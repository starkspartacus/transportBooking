...
const DynamicCaissierLoginForm = dynamic(
  () => import("@/components/auth/caissier-login-form").then((mod) => ({ default: mod.CaissierLoginForm })),
  {
    ssr: false,
    loading: () => (
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Button variant="ghost" onClick={handleBackToCountry} className="mb-4 text-slate-600 hover:text-slate-900">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Changer de pays
          </Button>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Connexion</h1>
          <p className="text-slate-600">
            Pays sélectionné : {AFRICAN_COUNTRIES.find((c) => c.code === selectedCountry)?.flag}{" "}
...
function CountrySelectionStep({ onCountrySelect }: { onCountrySelect: (country: string) => void }) {
  const [selectedCountry, setSelectedCountry] = useState<string>("")
      ...
      <div className="w-full max-w-md">
        <Card>
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-slate-900">Sélectionnez votre pays</CardTitle>
            <CardDescription>
              Pour votre sécurité, veuillez indiquer votre pays de résidence avant de vous connecter
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">Pays de résidence *</label>
            ...
            <div className="text-center">
              <p className="text-xs text-slate-500">Cette vérification garantit la sécurité de votre compte</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {loginMethod === "email" ? (
            ...
            <div className="flex">
              <div className="flex items-center px-3 bg-gray-100 border border-r-0 rounded-l-md">
                <span className="text-sm font-medium">{countryData?.phonePrefix}</span>
              </div>
              <input
                id="identifier"
                name="identifier"
