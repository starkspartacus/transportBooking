...
export default function TripFleetManagement({ companyId }: TripFleetManagementProps) {
  ...
  const fetchRoutes = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setRoutes(Array.isArray(data) ? data.filter((route: RouteData) => route.status === "ACTIVE") : [])
      }
    } catch (error) {
  ...
  const fetchBuses = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setBuses(Array.isArray(data) ? data.filter((bus: BusData) => bus.status === "ACTIVE") : [])
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "❌ Erreur",
          description: error.error || "Erreur lors de la programmation du voyage",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    const colorClasses = {
      ...
      yellow: "bg-yellow-100 text-yellow-800 border-yellow-200 hover:bg-yellow-200 transition-colors",
      green: "bg-green-100 text-green-800 border-green-200 hover:bg-green-200 transition-colors",
      ...
      orange: "bg-orange-100 text-orange-800 border-orange-200 hover:bg-orange-200 transition-colors",
    }
  ...
  const filteredTrips = trips.filter((trip) => {
    const matchesSearch =
      ...
      trip.route?.departureLocation?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      trip.route?.arrivalLocation?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      trip.bus?.plateNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      trip.driverName?.toLowerCase().includes(searchTerm.toLowerCase())

    const matchesStatus = !filterStatus || filterStatus === "_all" || trip.status === filterStatus
    const matchesType = !filterType || filterType === "_all" || trip.tripType === filterType
  ...
  const activeTrips = trips.filter((trip) => ["BOARDING", "DEPARTED", "IN_TRANSIT"].includes(trip.status))
  const completedTrips = trips.filter((trip) => ["ARRIVED", "COMPLETED"].includes(trip.status))
  ...
  const getTabTrips = (tab: string) => {
    switch (tab) {
      ...
      case "active":
        return filteredTrips.filter((trip) => ["BOARDING", "DEPARTED", "IN_TRANSIT"].includes(trip.status))
      case "completed":
        return filteredTrips.filter((trip) => ["ARRIVED", "COMPLETED"].includes(trip.status))
      case "cancelled":
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Gestion des Voyages</h2>
          ...
          <Dialog open={showNewForm} onOpenChange={setShowNewForm}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>Remplissez les informations pour programmer un nouveau voyage</DialogDescription>
              </DialogHeader>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            {routes.map((route) => (
                              <SelectItem key={route.id} value={route.id}>
                                <div className="flex items-center gap-2">
                                  ...
                                  {route.name} ({route.departureLocation} → {route.arrivalLocation})
                                </div>
                              </SelectItem>
                            ))}
                          </SelectContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium mb-2">Prix du billet (XOF) *</label>
                          <Input
                            type="number"
                            ...
                            return selectedRoute ? (
                              <div className="text-sm text-gray-600 space-y-1">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>Durée estimée: {formatDuration(selectedRoute.estimatedDuration)}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <DollarSign className="h-3 w-3" />
                                  <span>Prix de base: {selectedRoute.basePrice.toLocaleString()} XOF</span>
                                </div>
                              </div>
                            ) : null
                          })()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card className="bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200 hover:shadow-lg transition-all duration-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-3xl font-bold text-yellow-700">{scheduledTrips.length}</p>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200 hover:shadow-lg transition-all duration-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-3xl font-bold text-green-700">{activeTrips.length}</p>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 hover:shadow-lg transition-all duration-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-3xl font-bold text-purple-700">{completedTrips.length}</p>
                ...
                {isLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">
                      {trips.length === 0 ? "Aucun voyage trouvé" : `Aucun voyage ${tab === "all" ? "" : tab}`}
                    </h3>
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                {trip.services && trip.services.length > 0 && (
                                  ...
                                  >
                                    ...
                                    {trip.services.length} service{trip.services.length > 1 ? "s" : ""}
                                  </Badge>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="space-y-3">
                              ...
                              <div className="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                {trip.driverPhone && (
                                  <div className="flex items-center gap-1 ml-2">
                                    ...
                                    <span className="text-xs">{trip.driverPhone}</span>
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              <div className="space-y-1">
                                ...
                                <p className="font-semibold text-gray-900">{formatDateTime(trip.departureTime)}</p>
                              </div>
                              <div className="space-y-1">
                                <p className="text-gray-600 flex items-center gap-1">
                                  <CalendarIcon className="h-3 w-3" />
                                ...
                                <p className="font-semibold text-gray-900">{formatDateTime(trip.arrivalTime)}</p>
                              </div>
                            </div>

                            <div className="grid grid-cols-3 gap-3 text-sm">
                              <div className="text-center p-3 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
                                <p className="font-semibold text-blue-700">{trip.currentPrice.toLocaleString()}</p>
                                <p className="text-blue-600 text-xs">XOF</p>
                              </div>
                              <div className="text-center p-3 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                                <p className="font-semibold text-green-700">{trip.availableSeats}</p>
                                <p className="text-green-600 text-xs">Libres</p>
                              </div>
                              <div className="text-center p-3 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
                                <p className="font-semibold text-purple-700">{trip.bus?.capacity || 0}</p>
                            ...
                            {trip.services && trip.services.length > 0 && (
                              <div className="text-sm">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {trip.services.slice(0, 3).map((serviceId) => {
                                    const service = TRIP_SERVICES.find((s) => s.id === serviceId)
                                    return service ? (
                                      <Badge
                                        key={serviceId}
                                        variant="secondary"
                            ...
                            {trip._count && (
                              <div className="grid grid-cols-2 gap-3 text-sm">
                                <div className="text-center p-2 bg-blue-50 rounded border border-blue-200">
                                  <p className="font-semibold text-blue-700">{trip._count.reservations}</p>
                                  <p className="text-blue-600 text-xs">Réservations</p>
                                </div>
                                <div className="text-center p-2 bg-green-50 rounded border border-green-200">
                                  <p className="font-semibold text-green-700">{trip._count.tickets}</p>
                                  <p className="text-green-600 text-xs">Billets vendus</p>
                                </div>
                              </div>
                            )}
                            ...
                            {trip.status === "SCHEDULED" && (
                              <div className="flex gap-2 pt-2 border-t">
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleStatusChange(trip.id, "BOARDING")}
                            ...
                            {trip.status === "BOARDING" && (
                              <div className="flex gap-2 pt-2 border-t">
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleStatusChange(trip.id, "DEPARTED")}
                            ...
                            {trip.status === "DEPARTED" && (
                              <div className="flex gap-2 pt-2 border-t">
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleStatusChange(trip.id, "ARRIVED")}
      ...
      <Dialog open={showDetails} onOpenChange={setShowDetails}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Informations complètes sur le voyage sélectionné</DialogDescription>
          </DialogHeader>
          ...
          {selectedTrip && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{selectedTrip.route?.name}</p>
                        <p className="text-sm text-gray-600">
                          {selectedTrip.route?.departureLocation} → {selectedTrip.route?.arrivalLocation}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{selectedTrip.bus?.model}</p>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {selectedTrip.driverPhone && (
                            <p className="text-sm text-gray-600">{selectedTrip.driverPhone}</p>
                          )}
                        </div>
                      </div>
                    )}
                ...
                <div className="space-y-4">
                  <h3 className="font-semibold text-lg border-b pb-2">Horaires et tarifs</h3>
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-600">Départ</p>
                        <p className="font-medium">{formatDateTime(selectedTrip.departureTime)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Arrivée</p>
                        <p className="font-medium">{formatDateTime(selectedTrip.arrivalTime)}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <p className="text-sm text-gray-600">Prix</p>
                        <p className="font-medium">{selectedTrip.currentPrice.toLocaleString()} XOF</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">{selectedTrip.bus?.capacity}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selectedTrip.services && selectedTrip.services.length > 0 && (
                <div>
                  <h3 className="font-semibold text-lg border-b pb-2 mb-4">Services inclus</h3>
